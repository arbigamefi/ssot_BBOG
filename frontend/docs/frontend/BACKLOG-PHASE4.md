...
**Why**: No `.gitignore` at `frontend/` root. Risk of committing `node_modules/`, `.next/`, `.env.local`, IDE files. No env var documentation for developer onboarding.
...
**Files**:
- `frontend/package.json` — husky + lint-staged
- `frontend/.husky/pre-commit` — NEW
- `frontend/.lintstagedrc.json` — NEW
