...
type ThemeColor = 'purple' | 'amber' | 'emerald' | 'fuchsia' | 'blue';

interface ImmersiveGameLayoutProps {
   gameName: ReactNode;
...
export function ImmersiveGameLayout({
   gameName,
   themeColor,
   houseEdge,
   maxPayout,
   ...
   const getThemeClasses = () => {
      switch(themeColor) {
         ...
         case 'blue': return {
            ...
            glowTop: 'bg-blue-900/10',
            glowBot: 'bg-cyan-900/10',
            edgeColor: 'text-blue-400',
            leftPaneGrad: 'from-blue-900/10',
            rightPaneGlow: 'from-blue-500/10 via-cyan-600/5'
         };
         case 'purple': 
         default: return {
   ...
   return (
    <div className={cn("min-h-[calc(100vh-80px)] text-white font-sans overflow-x-hidden", t.selection)}>
      ...
      <div className={cn("fixed top-[-20%] right-[-10%] w-[800px] h-[800px] blur-[200px] rounded-full pointer-events-none z-0", t.glowTop)} />
      <div className={cn("fixed bottom-[-20%] left-[-10%] w-[600px] h-[600px] blur-[150px] rounded-full pointer-events-none z-0", t.glowBot)} />
      ...
      <main className="relative z-10 max-w-[1600px] mx-auto px-4 md:px-8 py-6 flex flex-col gap-6">
        ...
        <div className="flex justify-between items-end mb-2">
           <div>
              <div className="flex items-center gap-3 mb-2">
                 ...
                 <span className="text-xs font-bold uppercase tracking-widest text-green-400">Live SSOT Module</span>
              </div>
           ...
           <div className="hidden md:flex gap-6 text-right">
              <div className="flex flex-col">
                 <span className="text-[10px] text-white/40 uppercase tracking-widest font-bold">House Edge</span>
                 <span className={cn("text-lg font-mono", t.edgeColor)}>{houseEdge}</span>
              </div>
              <div className="flex flex-col">
                 <span className="text-[10px] text-white/40 uppercase tracking-widest font-bold">Max Payout</span>
        ...
        <div className="flex flex-col lg:flex-row w-full lg:min-h-[700px] rounded-[2.5rem] border border-white/10 bg-black/40 backdrop-blur-3xl shadow-[0_0_80px_rgba(0,0,0,0.8)] overflow-hidden">
           ...
           <div className={cn("w-full lg:w-[420px] xl:w-[450px] flex-shrink-0 bg-gradient-to-b to-transparent border-r border-white/10 p-6 md:p-8 flex flex-col relative z-20 overflow-y-auto hide-scrollbar", t.leftPaneGrad)}>
              {leftPaneContent}
           </div>

           {/* RIGHT PANE: THE MAIN STAGE */}
           <div className={cn("flex-1 relative flex flex-col items-center justify-center p-4 md:p-8 bg-[#030303] overflow-hidden group", isInteractive ? "" : "pointer-events-none")}>
              {/* Massive Stage Lighting */}
              <div className={cn("absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[800px] bg-gradient-to-b to-transparent blur-[100px] rounded-full pointer-events-none transition-all duration-1000 group-hover:scale-[1.05]", t.rightPaneGlow)} />
