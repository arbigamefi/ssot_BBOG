...
export default function DiceRoomPrototype3() {
  ...
  const isWin = rollResult !== null && (rollType === "under" ? rollResult <= targetNumber : rollResult >= targetNumber);
    ...
    <>
      ...
      <div className="grid grid-cols-2 gap-4 mb-auto">
         <div className="bg-[#050505] p-4 rounded-2xl border border-white/10 flex flex-col shadow-inner select-none transition-colors">
            ...
            <span className="text-2xl font-mono font-bold text-purple-400">{currentMultiplier.toFixed(3)}x</span>
         </div>
         <div className="bg-[#050505] p-4 rounded-2xl border border-white/10 flex flex-col shadow-inner select-none transition-colors">
         ...
         <div className="col-span-2 bg-[#050505] p-4 rounded-2xl border border-white/10 flex flex-col shadow-[inset_0_2px_15px_rgba(0,0,0,0.5)] select-none">
            ...
            <span className="text-3xl font-mono font-extrabold text-emerald-400 flex items-baseline gap-2">
               {expectedPayout.toFixed(2)} <span className="text-sm font-bold text-emerald-500/50">USDC</span>
            </span>
         </div>
      </div>
      ...
      <button 
         ...
         className={cn(
            ...
            isRolling || winChance === 0 ? "bg-purple-900 border-purple-900 shadow-none hover:shadow-none opacity-50 cursor-not-allowed" : "bg-purple-600 hover:bg-purple-500 border-purple-400"
         )}>
  ...
  const DiceDot = () => <div className="w-3 h-3 md:w-5 md:h-5 bg-gradient-to-br from-white to-gray-300 rounded-full shadow-[inset_0_2px_4px_rgba(0,0,0,0.5),0_0_10px_rgba(255,255,255,1)]" />;
  
  const DiceFace = ({ type, className }: { type: 1|2|3|4|5|6, className: string }) => {
     return (
        <div className={cn("absolute inset-0 bg-gradient-to-br from-purple-500 to-purple-900 border-[3px] border-purple-400/80 rounded-[2rem] shadow-[inset_0_0_50px_rgba(0,0,0,0.9),0_0_20px_rgba(168,85,247,0.5)] flex items-center justify-center backface-hidden backdrop-blur-sm", className)}>
    ...
    <>
      ...
      <div className="absolute top-6 right-6 lg:top-8 lg:right-8 z-20 hidden sm:block">
         <div className="flex flex-col items-end gap-2 p-3 rounded-2xl border border-white/5 bg-[#050505]/80 backdrop-blur-xl shadow-2xl">
            <div className="text-[10px] font-bold text-white/30 tracking-widest uppercase px-2">Recent Rolls</div>
            <div className="flex gap-2">
               {[42, 89, 12, 77, 50].map((res, i) => (
                  <div key={i} className={cn(
                    ...
                    (rollType === "under" ? res <= targetNumber : res >= targetNumber) ? "bg-emerald-500/10 text-emerald-400 border-emerald-500/30 shadow-[0_0_15px_rgba(16,185,129,0.1)]" : "bg-red-500/10 text-red-500 border-red-500/20"
                  )}>
                    {res}
                  </div>
               ))}
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center p-8 z-10 pointer-events-none">
         ...
         {/* The 3D Dice */}
         <div className={cn("relative flex items-center justify-center transition-all duration-500 mt-[-100px]", isRolling ? "" : "hover:scale-105")}>
            
            {/* Ambient Base Light */}
            <div className="absolute -bottom-12 w-64 h-16 bg-purple-500/30 blur-[40px] rounded-full pointer-events-none" />
            
            <div className="w-32 h-32 md:w-48 md:h-48 relative animate-[spin_30s_linear_infinite]" style={{ perspective: '1200px' }}>
               <div className={cn("w-full h-full relative", isRolling ? "animate-dice-roll-3d" : "")} style={{ transformStyle: 'preserve-3d', transform: 'rotateX(-20deg) rotateY(30deg)' }}>
         ...
         <div className={cn(
            ...
            isRolling ? "opacity-30 blur-sm pointer-events-none scale-95 transform translate-y-4" : "opacity-100"
         )}>
            ...
            <div className="flex items-center justify-between">
               ...
               <div className="flex bg-white/5 p-1 rounded-2xl border border-white/10 relative">
                  <div 
                     className={cn("absolute inset-y-1 w-[calc(50%-4px)] rounded-xl transition-all duration-300 shadow-[0_0_20px_rgba(0,0,0,0.5)]", rollType === "under" ? "bg-purple-600 left-1" : "bg-purple-600 left-[calc(50%+2px)]")} 
                  />
                  <button onClick={() => setRollType("under")} className={cn("px-6 py-2.5 rounded-xl font-bold uppercase tracking-wider text-xs relative z-10 transition-colors", rollType === "under" ? "text-white" : "text-white/40 hover:text-white")}>Roll Under</button>
                  <button onClick={() => setRollType("over")} className={cn("px-6 py-2.5 rounded-xl font-bold uppercase tracking-wider text-xs relative z-10 transition-colors", rollType === "over" ? "text-white" : "text-white/40 hover:text-white")}>Roll Over</button>
               </div>

               {/* Right: Selected Target Display */}
               <div className="flex items-center gap-4">
                  <div className="text-right">
                     <span className="block text-[10px] text-white/40 uppercase tracking-widest font-bold">Target Range</span>
         ...
         <div className="absolute inset-0 z-50 bg-black/80 backdrop-blur-md flex flex-col items-center justify-center animate-in fade-in zoom-in duration-300">
            <div className="p-10 rounded-[3rem] border border-white/10 bg-[#050505] shadow-[0_0_100px_rgba(0,0,0,1)] flex flex-col items-center text-center max-w-sm w-full relative overflow-hidden">
               ...
               <div className="relative z-10 flex flex-col items-center">
                  {isWin ? (
                     <>
                        <span className="text-emerald-400 font-bold mb-2 flex items-center gap-2"><div className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse"/> TARGET HIT</span>
                        <span className="text-4xl font-mono text-emerald-300 font-extrabold">+{expectedPayout.toFixed(2)} <span className="text-lg">USDC</span></span>
                     </>
                  ) : (
                     <>
                        <span className="text-red-500 font-bold mb-2 flex items-center gap-2"><div className="w-2 h-2 rounded-full bg-red-500"/> TARGET MISSED</span>
   ...
   <AuditTabs activeColorClass="border-purple-400 text-purple-400">
      ...
      <AuditTableRow className="hover:bg-white/[0.02] transition-colors border-b border-white/5 py-4 px-4">
         <div className="grid grid-cols-[1fr_1.5fr_1.5fr_1fr_80px] items-center">
           ...
           <AuditTableCell><span className="font-bold text-white/80">Precision Dice</span></AuditTableCell>
           <AuditTableCell><div className="flex flex-col"><span className="text-purple-300 font-bold font-mono text-xs mb-1">{rollType === "under" ? `UNDER ${targetNumber}` : `OVER ${targetNumber}`}</span><span className="text-xs text-white/50 font-mono">{betAmount} USDC</span></div></AuditTableCell>
           <AuditTableCell><span className="py-1.5 px-3 rounded-lg bg-purple-500/10 border border-purple-500/20 text-purple-400 font-mono text-[10px] uppercase font-bold animate-pulse">Awaiting VRF...</span></AuditTableCell>
