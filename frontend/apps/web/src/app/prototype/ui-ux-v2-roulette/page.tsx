    ...
    <>
      ...
      <div className="mb-6 bg-[#050505] rounded-2xl border border-white/10 p-4 min-h-[120px]">
         <div className="flex justify-between items-center mb-3">
            <label className="text-[10px] uppercase tracking-widest font-bold text-emerald-400 block">Selected Targets ({selectedSpots.length})</label>
            <button onClick={() => setSelectedSpots([])} className="text-xs font-bold text-white/40 hover:text-white transition-colors">Clear All</button>
         </div>
         ...
         <div className="flex flex-wrap gap-2">
            {selectedSpots.map(spot => {
               ...
               return (
                  <div key={spot} className={cn("px-3 py-1.5 rounded flex items-center justify-center text-xs font-bold font-mono border-b-2 border-white/20 shadow-inner", c)}>
                     {spot}
                  </div>
               );
            })}
            {selectedSpots.length === 0 && (
               <span className="text-xs text-white/20 italic">No targets selected. Click the board.</span>
            )}
         </div>
      </div>

      <div className="mb-6">
         <label className="text-[10px] uppercase tracking-widest font-bold text-white/40 mb-2 block">Bet Amount (Total)</label>
      ...
      <div className="grid grid-cols-2 gap-4 mb-auto">
         <div className="bg-[#050505] p-4 rounded-2xl border border-white/10 flex flex-col shadow-inner">
            <span className="text-[10px] uppercase font-bold text-white/40 tracking-widest mb-1 items-center flex gap-1">Net Win Chance <ChartBarIcon className="w-3 h-3"/></span>
            <span className="text-2xl font-mono font-bold text-white">{(selectedSpots.length * (100/37)).toFixed(2)}%</span>
         </div>
         <div className="bg-[#050505] p-4 rounded-2xl border border-white/10 flex flex-col shadow-inner">
            <span className="text-[10px] uppercase font-bold text-white/40 tracking-widest mb-1 items-center flex gap-1">Max Payout <InformationCircleIcon className="w-3 h-3"/></span>
      ...
      <button 
         ...
         className={cn(
            "mt-8 w-full py-6 rounded-2xl text-black font-extrabold text-xl shadow-[0_0_40px_rgba(16,185,129,0.3)] transition-all active:scale-[0.98] border-b-[4px] active:border-b hover:shadow-[0_0_60px_rgba(16,185,129,0.5)]",
             selectedSpots.length === 0 ? "bg-emerald-900 text-white/30 border-emerald-900 shadow-none hover:shadow-none" : "bg-emerald-600 hover:bg-emerald-500 border-emerald-400"
         )}>
    ...
    <>
      <div className="absolute top-6 right-6 lg:top-8 lg:right-8 z-20 hidden md:block">
         <div className="flex flex-col items-end gap-2 p-3 rounded-2xl border border-white/5 bg-[#050505]/80 backdrop-blur-xl shadow-2xl">
            <div className="text-[10px] font-bold text-white/30 tracking-widest uppercase px-2">Recent Numbers</div>
            <div className="flex gap-2">
               {[ {num: 12, c: 'red'}, {num: 0, c: 'green'}, {num: 35, c: 'black'} ].map((res, i) => (
                  <div key={i} className={cn(
                    "w-8 h-8 rounded-full flex items-center justify-center text-xs font-mono font-bold border",
                    res.c === 'red' ? "bg-red-600 text-white border-red-400 shadow-[0_0_15px_rgba(220,38,38,0.3)]" : 
                    res.c === 'green' ? "bg-emerald-500 text-white border-emerald-300 shadow-[0_0_15px_rgba(16,185,129,0.3)]" : 
         ...
         <div className="w-[300px] h-[300px] md:w-[380px] md:h-[380px] rounded-full border-[12px] md:border-[16px] border-[#2C1810] shadow-[0_0_60px_rgba(0,0,0,1),inset_0_0_20px_black] ring-4 ring-[#B8860B] flex items-center justify-center p-1 md:p-2 relative bg-[#111]">
            ...
            <div 
               className={cn(
                  ...
                  isSpinning ? "animate-[spin_4s_cubic-bezier(0.1,0.7,0.1,1)_forwards] blur-[1px]" : "rotate-0"
               )}
               style={{
                  background: `conic-gradient(from -${360/37/2}deg, ${europeanWheelOrder.map((num, i) => {
            ...
            >
               <div className={cn(
                  ...
                  isSpinning ? "top-[10px] md:top-[15px] shadow-[0_0_15px_white,-10px_0px_10px_black] scale-125 blur-[1px] duration-[2000ms]" : "top-[42px] md:top-[55px] shadow-[0_0_10px_white,-4px_4px_10px_black] scale-100 duration-1000"
               )} />
            </div>
         ...
         <div className="flex bg-[#0B1A12] border-4 border-[#222] rounded-xl p-3 md:p-4 shadow-[0_20px_50px_rgba(0,0,0,0.8),inset_0_0_40px_rgba(0,0,0,0.8)] relative overflow-hidden">
            ...
            <button onClick={() => toggleSpot("0")}
               className={cn(
                  ...
                  selectedSpots.includes("0") ? "bg-emerald-400 border-emerald-300 text-black shadow-[0_0_30px_rgba(52,211,153,0.8),inset_0_2px_5px_rgba(255,255,255,0.8)] z-10 scale-[1.03] rounded-lg" : "bg-[#093d25] border-[#105e3a] text-emerald-100 hover:bg-[#0c4e30] border-r-0 shadow-[inset_0_2px_0_rgba(255,255,255,0.1),inset_0_-2px_0_rgba(0,0,0,0.3)]"
               )}>0</button>
               ...
               {gridRows.map((row, rI) => (
                  <div key={rI} className="flex gap-[4px]">
                     {row.map(num => {
                        ...
                        return (
                           <button key={num} onClick={() => toggleSpot(num.toString())}
                              className={cn(
                                 ...
                                 isSelected && isRed ? "bg-red-500 border-red-300 text-white shadow-[0_0_30px_rgba(239,68,68,0.8),inset_0_2px_5px_rgba(255,255,255,0.8)] z-10 scale-[1.1] rounded-lg font-extrabold" : 
                                 isSelected && !isRed ? "bg-white border-gray-300 text-black shadow-[0_0_30px_rgba(255,255,255,0.9),inset_0_2px_5px_rgba(255,255,255,1)] z-10 scale-[1.1] rounded-lg font-extrabold" :
                                 isRed ? "bg-[#7f1d1d] hover:bg-[#991b1b] text-red-100 border-[#991b1b] shadow-[inset_0_2px_0_rgba(255,255,255,0.1),inset_0_-2px_0_rgba(0,0,0,0.4)]" : "bg-[#1f2937] hover:bg-[#374151] text-gray-200 border-[#374151] shadow-[inset_0_2px_0_rgba(255,255,255,0.1),inset_0_-2px_0_rgba(0,0,0,0.4)]"
                              )}>{num}</button>
                        );
                     })}
                     <button onClick={() => toggleSpot(`ROW-${rI}`)}
                        className={cn(
                           "w-10 md:w-14 border font-bold text-[8px] md:text-[10px] uppercase font-mono transition-all rounded-sm relative",
                           selectedSpots.includes(`ROW-${rI}`) ? "bg-blue-500 border-blue-300 text-white shadow-[0_0_30px_rgba(59,130,246,0.8),inset_0_2px_5px_rgba(255,255,255,0.8)] z-10 scale-[1.1] rounded-lg" : "bg-white/5 hover:bg-white/10 border-white/10 text-white/50 shadow-[inset_0_2px_0_rgba(255,255,255,0.05),inset_0_-2px_0_rgba(0,0,0,0.2)]"
                        )}>2:1</button>
                  </div>
               ))}
            </div>
         ...
         <div className="flex flex-col gap-[4px] mt-[4px] ml-12 md:ml-[72px] mr-10 md:mr-14">
            <div className="flex gap-[4px]">
               {['1st 12', '2nd 12', '3rd 12'].map((doz, i) => (
                  <button key={i} onClick={() => toggleSpot(doz)} className={cn("flex-1 py-2 md:py-3 border font-bold text-xs uppercase transition-all rounded-sm", selectedSpots.includes(doz) ? "bg-blue-500 border-blue-300 text-white shadow-[0_0_30px_rgba(59,130,246,0.8),inset_0_2px_5px_rgba(255,255,255,0.8)] z-10 scale-[1.05] rounded-md" : "bg-white/5 hover:bg-white/10 border-white/10 text-white/50 shadow-[inset_0_2px_0_rgba(255,255,255,0.05),inset_0_-2px_0_rgba(0,0,0,0.2)]")}>{doz}</button>
               ))}
            </div>
            <div className="flex gap-[4px]">
               {['1-18', 'EVEN', 'RED', 'BLACK', 'ODD', '19-36'].map(out => {
                  let c = selectedSpots.includes(out) ? "bg-blue-500 border-blue-300 text-white shadow-[0_0_30px_rgba(59,130,246,0.8),inset_0_2px_5px_rgba(255,255,255,0.8)] z-10 scale-[1.05] rounded-md" : "bg-white/5 hover:bg-white/10 border-white/10 text-white/50 shadow-[inset_0_2px_0_rgba(255,255,255,0.05),inset_0_-2px_0_rgba(0,0,0,0.2)]";
                  if (out === 'RED') c = selectedSpots.includes(out) ? "bg-red-500 border-red-300 text-white shadow-[0_0_30px_rgba(239,68,68,0.8),inset_0_2px_5px_rgba(255,255,255,0.5)] z-10 scale-[1.05] rounded-md" : "bg-[#7f1d1d]/40 text-red-400 border-red-900/50 hover:bg-[#991b1b]/50 shadow-[inset_0_2px_0_rgba(255,255,255,0.05),inset_0_-2px_0_rgba(0,0,0,0.2)]";
                  if (out === 'BLACK') c = selectedSpots.includes(out) ? "bg-white border-gray-300 text-black shadow-[0_0_30px_rgba(255,255,255,0.8),inset_0_2px_5px_rgba(255,255,255,1)] z-10 scale-[1.05] rounded-md" : "bg-[#1f2937]/40 text-gray-400 border-gray-700/50 hover:bg-[#374151]/50 shadow-[inset_0_2px_0_rgba(255,255,255,0.05),inset_0_-2px_0_rgba(0,0,0,0.2)]";
                  return (
                     <button key={out} onClick={() => toggleSpot(out)} className={cn("flex-1 py-3 border font-bold text-xs md:text-sm uppercase flex items-center justify-center transition-all rounded-sm", c)}>
                        {out === 'RED' ? <div className="w-5 h-5 bg-red-600 rounded-sm shadow-[inset_0_1px_2px_rgba(0,0,0,0.8)] border border-red-800" /> : out === 'BLACK' ? <div className="w-5 h-5 bg-[#111] rounded-sm shadow-[inset_0_1px_2px_rgba(0,0,0,0.8)] border border-gray-800" /> : out}
                     </button>
                  );
               })}
            </div>
      ...
      {showResult && resultNum !== null && (
         <div className="absolute inset-0 z-30 bg-black/60 backdrop-blur-sm flex flex-col items-center justify-center animate-in fade-in zoom-in duration-300">
            <div className="p-8 rounded-[2rem] border border-white/10 bg-[#050505] shadow-2xl flex flex-col items-center text-center">
               <h3 className="text-xl font-bold text-white/60 uppercase tracking-widest mb-2">Winning Number</h3>
               <div className={cn(
                  "text-6xl font-extrabold mb-4 font-mono w-24 h-24 rounded-full flex items-center justify-center border-4 shadow-2xl", 
                  resultNum === 0 ? "bg-emerald-500 border-emerald-300 text-white shadow-[0_0_30px_rgba(16,185,129,0.5)]" : 
                  redNumbers.includes(resultNum) ? "bg-red-600 border-red-300 text-white shadow-[0_0_30px_rgba(220,38,38,0.5)]" : 
               ...
               {isWin ? (
                  <div className="flex flex-col items-center mt-2">
                     ...
                     <span className="text-3xl font-mono text-emerald-300">+{(parseFloat(betAmount)*36).toFixed(2)} USDC</span>
                  </div>
               ) : (
                  <div className="flex flex-col items-center mt-2">
                     <span className="text-red-500 font-bold mb-1">NO DIRECT MATCH</span>
                     <span className="text-sm font-mono text-white/40">(Evaluating outside bets via contract...)</span>
                  </div>
               )}
            </div>
         </div>
   ...
   <AuditTabs activeColorClass="border-emerald-400 text-emerald-400">
      ...
      <AuditTableRow className="hover:bg-white/[0.02] transition-colors border-b border-white/5 py-4 px-4">
         <div className="grid grid-cols-[1fr_1.5fr_1.5fr_1fr_80px] items-center">
           ...
           <AuditTableCell><span className="font-bold text-white/80">Roulette</span></AuditTableCell>
           <AuditTableCell><div className="flex flex-col"><span className="text-emerald-300 font-bold font-mono text-[10px] mb-1 truncate">SPOTS: {selectedSpots.length}</span><span className="text-xs text-white/50 font-mono">20.00 USDC</span></div></AuditTableCell>
           <AuditTableCell><span className="py-1.5 px-3 rounded-lg bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 font-mono text-[10px] uppercase font-bold animate-pulse">Awaiting VRF...</span></AuditTableCell>
