          ...
          {reels.map((symbol, index) => {
            ...
            return (
              <div
                key={index}
                className={cn(
                  "flex h-36 flex-col items-center justify-center rounded-lg border bg-surface-2 shadow-inner-e1 md:h-40",
                  isPending && "animate-pulse border-brand/40",
                  hasResult && "border-accent/50 bg-accent-soft"
                )}
              >
                <span className="font-mono text-5xl font-semibold text-fg">{meta.mark}</span>
