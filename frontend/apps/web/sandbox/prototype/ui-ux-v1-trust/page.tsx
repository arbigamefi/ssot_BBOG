      ...
      right={
        <div className="grid grid-cols-3 gap-4">
          <MetricTile
            label="NAV"
            value="$4.82M"
            note="LP backing after protocol and XP liabilities."
            tone="accent"
          />
          ...
          <MetricTile
            label="Exit room"
            value="$2.74M"
            note="Currently available for withdraw / redeem."
          />
        </div>
      }
    >
        ...
        <div className="space-y-5">
          <div className="rounded-[1.8rem] border border-white/10 bg-white/[0.03] p-5">
            ...
            <div className="mt-5 grid grid-cols-2 gap-4">
              <MetricTile
                label="Min liquidity floor"
                value="$970k"
                note="Protected floor that blocks optional outflow when breached."
              />
              <MetricTile
                label="Protocol fees payable"
                value="$184k"
                note="Not part of LP backing. Paid before NAV can be interpreted."
              />
              <MetricTile
                label="XP liabilities"
                value="$126k"
                note="Outstanding XP bucket exposure kept outside LP backing."
              />
              <MetricTile
                label="Free balance"
                value="$3.71M"
                note="Bank cash after reserved risk but before optional withdrawal limits."
              />
            </div>
          </div>
              ...
              <div className="rounded-[1.3rem] border border-white/10 bg-white/[0.04] p-4">
                ...
                <div className="mt-4 space-y-3">
                  {[
                    ...
                    <div
                      key={label}
                      className="flex items-center justify-between border-b border-white/8 pb-3 text-sm text-slate-300 last:border-b-0 last:pb-0"
                    >
                      <span>{label}</span>
              ...
              <div className="rounded-[1.3rem] border border-cyan-300/25 bg-[linear-gradient(180deg,rgba(11,27,50,0.96),rgba(6,15,33,0.98))] p-4">
                ...
                <div className="mt-4 rounded-[1rem] border border-white/10 bg-black/20 px-4 py-4">
                  ...
                  <div className="mt-2 text-sm leading-6 text-slate-400">
                    Deposits are open. Optional outflow remains healthy because reserve and floor
                    checks still leave room for exit.
                  </div>
                </div>
          ...
          <div className="rounded-[1.8rem] border border-white/10 bg-white/[0.03] p-5">
            ...
            <div className="mt-4 space-y-3">
              {[
                [
                  "NAV backs LPs",
                  "Treat NAV as the real backing number, not free balance or TVL shorthand."
                ],
                [
                  "Reserved is active exposure",
                  "It explains why cash can exist but still be unavailable for optional outflow."
                ],
                [
                  "XP and PF sit outside backing",
                  "Both liabilities are visible so LPs do not overread bank health."
                ]
              ].map(([title, desc]) => (
                <div
                  key={title}
          ...
          <div className="rounded-[1.8rem] border border-white/10 bg-white/[0.03] p-5">
            ...
            <div className="mt-4 space-y-3 text-sm text-slate-300">
              <div className="flex items-center justify-between">
                ...
                <span className="font-semibold text-white">0x8c7...22f</span>
              </div>
              <div className="flex items-center justify-between">
                <span>Chain</span>
              ...
              <div className="flex items-center justify-between">
                <span>Bank</span>
                <span className="font-semibold text-white">0x4D...b7</span>
              </div>
            </div>
      ...
      right={
        <div className="grid grid-cols-3 gap-4">
          <MetricTile
            label="Indexed bets"
            value="18,422"
            note="Event-backed list coverage across supported rooms."
            tone="accent"
          />
          ...
          <MetricTile
            label="Settled today"
            value="1,126"
            note="Recent settlement pace across all rooms."
          />
        </div>
      }
    >
            ...
            >
              ...
              <div>
                ...
                <div className="mt-1 text-xs text-slate-500">
                  Event-derived room line with decode-ready label.
                </div>
              </div>
              ...
              <div>
                <span className="rounded-full bg-white px-4 py-2 text-xs font-semibold text-slate-950">
                  {action}
                </span>
              </div>
            </div>
      ...
      right={
        <div className="grid grid-cols-3 gap-4">
          <MetricTile
            label="Room"
            value="Roulette"
            note="Standard European table room."
            tone="accent"
          />
          <MetricTile
            label="Status"
            value="Pending"
            note="VRF requested and still within timeout."
          />
          <MetricTile
            label="Action"
            value="Watch"
            note="Refund remains disabled until timeout passes."
          />
        </div>
      }
    >
          ...
          <div className="rounded-[1.8rem] border border-white/10 bg-white/[0.03] p-5">
            <div className="grid grid-cols-2 gap-4">
              <MetricTile
                label="Bet ID"
                value="#18212"
                note="Bound to tx journal for local self-audit."
                tone="accent"
              />
              <MetricTile
                label="Stake / fee"
                value="10.00 + 0.14"
                note="Stake and VRF fee remain visually distinct."
              />
            </div>
            <div className="mt-4 rounded-[1.2rem] border border-white/10 bg-[#071024] px-4 py-4">
              ...
              <div className="mt-2 text-base text-slate-300">
                Straight 17 on European roulette table, 1 round, USDC asset.
              </div>
            </div>
          </div>
          ...
          <div className="rounded-[1.8rem] border border-white/10 bg-[linear-gradient(180deg,rgba(8,17,36,0.98),rgba(7,12,24,0.98))] p-5">
            ...
            <div className="mt-4 space-y-3">
              {[
                [
                  "Placed",
                  "User signed placeBet. Ticket recorded in local journal and emitted on-chain."
                ],
                ...
                <div
                  key={title}
                  className="flex items-start gap-4 rounded-[1.1rem] border border-white/8 bg-[#071024] px-4 py-4"
                >
        ...
        <div className="space-y-5">
            ...
            <div className="mt-4 space-y-3">
              <div className="rounded-[1.1rem] border border-white/10 bg-white/[0.04] px-4 py-4">
                ...
                <div className="mt-1 text-sm leading-6 text-slate-400">
                  Disabled until the bet crosses refund timeout.
                </div>
              </div>
              <div className="rounded-[1.1rem] border border-white/10 bg-white/[0.04] px-4 py-4">
                <div className="text-sm font-semibold text-white">Bind bet ID</div>
                <div className="mt-1 text-sm leading-6 text-slate-400">
                  Available only when tx is known but event binding is still missing.
                </div>
              </div>
              <div className="rounded-full bg-white px-5 py-3 text-center text-sm font-semibold text-slate-950">
                Open tx trace
              </div>
            </div>
          </div>
          ...
          <div className="rounded-[1.8rem] border border-white/10 bg-white/[0.03] p-5">
            ...
            <div className="mt-4 space-y-3 text-sm text-slate-300">
              <div className="flex items-center justify-between">
                <span>Tx hash</span>
                <span className="font-semibold text-white">0x17...8c</span>
              </div>
              <div className="flex items-center justify-between">
                <span>Release digest</span>
                <span className="font-semibold text-white">0x8c7...22f</span>
              ...
              <div className="flex items-center justify-between">
                <span>Chain ID</span>
                <span className="font-semibold text-white">84532</span>
              </div>
            </div>
          </div>
        </div>
      ...
      right={
        <div className="grid grid-cols-3 gap-4">
          <MetricTile
            label="Wallet assets"
            value="3"
            note="Tracked assets with readable wallet and bank views."
            tone="accent"
          />
          <MetricTile
            label="Refund credit"
            value="0.23 ETH"
            note="Claimable VRF refund shown near identity context."
          />
          <MetricTile
            label="Journal rows"
            value="82"
            note="Local self-audit trail for actions and receipts."
          />
        </div>
      }
    >
          ...
          <div className="rounded-[1.8rem] border border-white/10 bg-white/[0.03] p-5">
            ...
            <div className="mt-4 grid grid-cols-3 gap-4">
              {[
                ...
                <div
                  key={asset}
                  className="rounded-[1.2rem] border border-white/10 bg-[#071024] p-4"
                >
          ...
          <div className="rounded-[1.8rem] border border-white/10 bg-[linear-gradient(180deg,rgba(8,17,36,0.98),rgba(7,12,24,0.98))] p-5">
            ...
            <div className="mt-4 overflow-hidden rounded-[1.2rem] border border-white/10">
              ...
              {[
                {
                  time: "12:41",
                  action: "Roulette bet placed · Straight 17",
                  chain: "84532",
                  digest: "0x8c7...22f"
                },
                {
                  time: "12:03",
                  action: "USDC deposit reviewed",
                  chain: "84532",
                  digest: "0x8c7...22f"
                },
                {
                  time: "11:22",
                  action: "Refund credit claimed",
                  chain: "84532",
                  digest: "0x8c7...22f"
                }
        ...
        <div className="space-y-5">
          <div className="rounded-[1.8rem] border border-cyan-300/25 bg-[linear-gradient(180deg,rgba(11,27,50,0.96),rgba(6,15,33,0.98))] p-5">
            ...
            <div className="mt-2 text-sm leading-6 text-slate-300">
              Claimable VRF refund credit remains readable and attached to wallet identity.
            </div>
          ...
          <div className="rounded-[1.8rem] border border-white/10 bg-white/[0.03] p-5">
            ...
            <div className="mt-4 space-y-3 text-sm text-slate-300">
              ...
              <div className="flex items-center justify-between">
                <span>Mode</span>
                <span className="font-semibold text-white">Canonical release</span>
              </div>
              <div className="flex items-center justify-between">
                <span>Hub</span>
          ...
          <div className="flex flex-wrap items-start justify-between gap-6">
            <div className="space-y-3">
              ...
              <h1 className="text-4xl font-black tracking-[-0.05em] text-white">
                ArbiGameFi UI/UX Prototype v1.1
              </h1>
              <p className="max-w-3xl text-base leading-7 text-slate-300">
                Trust and audit route boards for Figma capture. These screens translate release
                pack, LP onboarding, and route review work into calmer product surfaces.
              </p>
            </div>
