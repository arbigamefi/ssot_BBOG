...
import { CurrencyDollarIcon, WalletIcon, ChartBarIcon } from "@heroicons/react/24/outline";
...
export default function KenoRoomPrototype3() {
  ...
  React.useEffect(() => {
    ...
    if (isDrawing) {
      interval = setInterval(() => {
        ...
        const rnd: number[] = [];
        while (rnd.length < 8) {
          // Flash 8 random tiles per frame
    ...
    <>
      ...
      <div className="mb-6 flex flex-col gap-3 rounded-2xl bg-[#050505] border border-white/10 p-4">
        <div className="flex justify-between items-center">
          <div className="flex flex-col">
            <span className="text-[10px] uppercase font-bold text-fuchsia-400 tracking-widest">
              Spots Picked
            </span>
            <span className="text-xl font-bold font-mono">
              {selectedSpots.length} <span className="text-white/30 text-sm">/ 10</span>
            </span>
          </div>
          <div className="flex gap-2">
            <button
              onClick={autoPick}
              className="px-4 py-2 rounded-xl border border-fuchsia-500/50 bg-fuchsia-500/10 text-fuchsia-300 hover:bg-fuchsia-500/30 text-xs font-bold transition-all shadow-[0_0_15px_rgba(217,70,239,0.2)]"
            >
              Auto Pick
            </button>
            <button
              onClick={() => setSelectedSpots([])}
              className="px-4 py-2 rounded-xl bg-[#111] border border-white/5 text-white/40 hover:text-white hover:bg-[#222] text-xs font-bold transition-all shadow-inner"
            >
              Clear
            </button>
          </div>
        ...
        {selectedSpots.length > 0 ? (
          <div className="flex flex-wrap gap-1.5 mt-2">
            {selectedSpots
              ...
              .map((n) => (
                <div
                  key={n}
                  className="w-7 h-7 flex items-center justify-center rounded-md bg-fuchsia-600 text-white font-mono text-[10px] font-bold shadow-inner border border-fuchsia-400"
                >
                  {n}
                </div>
              ))}
          ...
          <div className="text-xs text-white/20 italic mt-2">
            Pick up to 10 numbers on the board.
          </div>
        )}
      </div>
      ...
      <div className="grid grid-cols-2 gap-4 mb-auto">
        <div className="bg-[#050505] p-4 rounded-2xl border border-white/10 flex flex-col shadow-inner col-span-2">
          <span className="text-[10px] uppercase font-bold text-white/40 tracking-widest mb-2 items-center flex gap-1">
            Hit Payouts ({selectedSpots.length} Picks) <ChartBarIcon className="w-3 h-3" />
          </span>
          {selectedSpots.length > 0 ? (
            <div className="flex justify-between text-xs font-mono font-bold">
              <span className="text-white/40 text-[10px]">1 Hit: 0x</span>
              <span className="text-white/60 text-[10px]">
                {(selectedSpots.length / 4).toFixed(0)} Hits: 2x
              </span>
              <span className="text-fuchsia-300 text-[10px]">
                {(selectedSpots.length / 2).toFixed(0)} Hits: 10x
              </span>
              <span className="text-fuchsia-400 text-[10px]">
                {selectedSpots.length} Hits: 100x
              </span>
            </div>
          ) : (
      ...
      <button
        ...
        className={cn(
          ...
          selectedSpots.length === 0
            ? "bg-fuchsia-900 border-fuchsia-900 text-white/30 shadow-none hover:shadow-none"
            : "bg-fuchsia-600 hover:bg-fuchsia-500 border-fuchsia-400"
        )}
      >
  ...
  const RightPane = (
    <>
      <div
        className={cn(
          "relative z-10 w-full max-w-[800px] bg-[#020202] rounded-[2.5rem] border border-white/5 p-6 md:p-8 shadow-[0_40px_100px_rgba(0,0,0,0.8),inset_0_2px_20px_rgba(255,255,255,0.02)] transition-all duration-700",
          isDrawing ? "scale-[0.98] blur-[0.5px]" : ""
        )}
      >
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  isHit
                    ? "bg-emerald-500 text-black font-extrabold shadow-[0_0_40px_rgba(16,185,129,1),inset_0_2px_10px_rgba(255,255,255,0.8)] border-[3px] border-emerald-200 scale-110 z-20 animate-pulse"
                    : isDrawn
                      ? "bg-[#222] text-white/40 border-b-[3px] border-[#111] scale-95 opacity-60 shadow-inner z-10"
                      : isAnimating
                        ? "bg-fuchsia-300 text-fuchsia-950 font-extrabold shadow-[0_0_30px_rgba(232,121,249,1),inset_0_2px_10px_rgba(255,255,255,0.9)] border-[3px] border-white scale-110 z-30"
                        : isSelected
                          ? "bg-gradient-to-br from-fuchsia-500 to-fuchsia-700 text-white shadow-[0_5px_20px_rgba(217,70,239,0.5),inset_0_2px_5px_rgba(255,255,255,0.4)] border-b-[4px] border-r-[2px] border-fuchsia-900 scale-105"
      ...
      {showResult && (
        <div className="absolute inset-0 z-30 bg-black/60 backdrop-blur-sm flex flex-col items-center justify-center animate-in fade-in zoom-in duration-300">
          <div className="p-8 rounded-[2rem] border border-white/10 bg-[#050505] shadow-2xl flex flex-col items-center text-center">
            ...
            <div className="text-5xl font-extrabold mb-4 text-fuchsia-400">
              {matchCount} <span className="text-2xl text-white/40">HITS</span>
            </div>
            {payout > 0 ? (
    ...
    <AuditTabs activeColorClass="border-fuchsia-400 text-fuchsia-400">
      ...
      <AuditTableRow className="hover:bg-white/[0.02] transition-colors border-b border-white/5 py-4 px-4">
        <div className="grid grid-cols-[1fr_1.5fr_1.5fr_1fr_80px] items-center">
          ...
          <AuditTableCell>
            <span className="font-bold text-white/80">Keno Draft</span>
          </AuditTableCell>
          <AuditTableCell>
            <div className="flex flex-col">
              <span className="text-fuchsia-300 font-bold font-mono text-[10px] mb-1 truncate">
                PICKS: {selectedSpots.length > 0 ? selectedSpots.join(",") : "N/A"}
              </span>
              <span className="text-xs text-white/50 font-mono">5.00 USDC</span>
            </div>
          </AuditTableCell>
          <AuditTableCell>
            <span className="py-1.5 px-3 rounded-lg bg-fuchsia-500/10 border border-fuchsia-500/20 text-fuchsia-400 font-mono text-[10px] uppercase font-bold animate-pulse">
              Awaiting VRF...
            </span>
          </AuditTableCell>
