...
export default function UITestPage() {
  ...
  const tableCols = [
    {
      key: "id",
      header: "ID",
      render: (r: any) => <span className="text-white/40 font-mono">{r.id}</span>
    },
    {
      key: "name",
      header: "Name",
      render: (r: any) => <span className="font-bold">{r.name}</span>
    },
    {
      key: "status",
      header: "Status",
      render: (r: any) => (
        <span className="text-emerald-400 drop-shadow-[0_0_5px_#10b981]">{r.status}</span>
      )
    }
  ];
  const tableData = [
  ...
  return (
    <CyberLayout
      headerNode={
        <CyberHeader appName="UI Test Lab" navLinks={mockHeaderLinks} activeRouteId="1" />
      }
    ...
    >
      ...
      <section>
        <h2 className="text-2xl font-bold mb-6 border-b border-white/10 pb-2">
          3. Molecules: Stat Blocks & Game Cards
        </h2>
      ...
      <section>
        <h2 className="text-2xl font-bold mb-6 border-b border-white/10 pb-2">
          4. Molecules: CyberTable & ReceiptTicket
        </h2>
      ...
      <section>
        <h2 className="text-2xl font-bold mb-6 border-b border-white/10 pb-2">
          5. Organisms: Win/Loss Overlays
        </h2>
        <div className="flex gap-4">
          <CyberButton onClick={() => triggerOverlay(true)}>Trigger WIN Overlay</CyberButton>
          <CyberButton variant="danger" onClick={() => triggerOverlay(false)}>
