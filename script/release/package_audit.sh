...
#   RELEASE_PATH   deployments/release-latest.json
#   SNAPSHOT_PATH  deployments/latest.json
#   NOTES_PATH     deployments/release-notes-latest.md
...
cp -f "$SNAPSHOT_PATH" "$BUNDLE_ROOT/deployments/latest.json"
cp -f "$RELEASE_PATH" "$BUNDLE_ROOT/deployments/release-latest.json"
cp -f "$NOTES_PATH" "$BUNDLE_ROOT/deployments/release-notes-latest.md"
cp -f "$FRONTEND_MANIFEST_PATH" "$BUNDLE_ROOT/deployments/frontend-manifest-latest.json"
cp -f "$GOLDEN_VECTORS_PATH" "$BUNDLE_ROOT/deployments/golden-vectors-latest.json"

SNAPSHOT_CONV="deployments/snapshots/deploy-${CHAIN_ID}-${BLOCK_NUMBER}.json"
[[ -f "$SNAPSHOT_CONV" ]] && cp -f "$SNAPSHOT_CONV" "$BUNDLE_ROOT/$SNAPSHOT_CONV"

RELEASE_CONV="deployments/release/release-${CHAIN_ID}-${BLOCK_NUMBER}.json"
[[ -f "$RELEASE_CONV" ]] && cp -f "$RELEASE_CONV" "$BUNDLE_ROOT/$RELEASE_CONV"

NOTES_CONV="deployments/release/release-notes-${CHAIN_ID}-${BLOCK_NUMBER}.md"
[[ -f "$NOTES_CONV" ]] && cp -f "$NOTES_CONV" "$BUNDLE_ROOT/$NOTES_CONV"

FRONTEND_CONV="deployments/release/frontend-manifest-${CHAIN_ID}-${BLOCK_NUMBER}.json"
[[ -f "$FRONTEND_CONV" ]] && cp -f "$FRONTEND_CONV" "$BUNDLE_ROOT/$FRONTEND_CONV"

VECTORS_CONV="deployments/release/golden-vectors-${CHAIN_ID}-${BLOCK_NUMBER}.json"
[[ -f "$VECTORS_CONV" ]] && cp -f "$VECTORS_CONV" "$BUNDLE_ROOT/$VECTORS_CONV"

VERIFY_LATEST="deployments/verify-latest.sh"
[[ -f "$VERIFY_LATEST" ]] && cp -f "$VERIFY_LATEST" "$BUNDLE_ROOT/$VERIFY_LATEST"

VERIFY_CONV="deployments/verify/verify-${CHAIN_ID}-${BLOCK_NUMBER}.sh"
...
\`\`\`bash
RELEASE_PATH=deployments/release-latest.json SNAPSHOT_PATH=deployments/latest.json \
