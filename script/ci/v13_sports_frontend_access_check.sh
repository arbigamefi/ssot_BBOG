...
# Validates SportsHub jurisdiction and frontend-access approval before public-money risk-in.
#
# Usage:
#   bash script/ci/v13_sports_frontend_access_check.sh [memo-json]
#   REQUIRE_APPROVED=1 bash script/ci/v13_sports_frontend_access_check.sh <approved-memo-json>
...
MEMO="${1:-${FRONTEND_ACCESS_FILE:-docs/ops/templates/sportsbook-frontend-access.example.json}}"
...
def require_filled(value, label):
    if not isinstance(value, str) or not value or value == "TBD":
        fail(f"{label} must be filled for approved frontend access")
...
if schema != "sportsbook.frontend-access.v1":
...
public_enabled = require_bool(
    require(frontend, "publicSportsbookEnabled", "frontend.publicSportsbookEnabled"),
    "frontend.publicSportsbookEnabled",
)
enablement_flag = require(frontend, "enablementFlag", "frontend.enablementFlag")
if not isinstance(enablement_flag, str) or not enablement_flag:
    fail("frontend.enablementFlag must be non-empty")
...
licenses = require(
    jurisdiction,
    "operatorLicensesOrExemptions",
    "jurisdictionDecision.operatorLicensesOrExemptions",
)
...
responsible_controls = require(
    access,
    "responsibleGamingControls",
    "accessControls.responsibleGamingControls",
)
...
for field in ["termsUri", "privacyUri", "riskDisclaimerUri", "rulebookUri"]:
...
for field in [
    ...
    "sportsbookEntryHiddenWhenDisabled",
    "serverSideOrWalletGateRequired",
    "auditLogRequired",
]:
...
if require_approved:
    ...
    for field in ["agePolicy", "kycPolicy", "sanctionsPolicy", "geoRestrictionProvider", "vpnProxyPolicy"]:
...
print("v1.3 Sports frontend-access approval memo validated:")
print(f"  memo: {path}")
print(f"  status: {status}")
print(f"  chainId: {chain_id}")
print(f"  sportsHub: {sports_hub}")
print(f"  publicSportsbookEnabled: {public_enabled}")
