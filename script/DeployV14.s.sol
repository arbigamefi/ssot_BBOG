...
interface IERC20MetadataLikeV14 {
    function symbol() external view returns (string memory);
    function decimals() external view returns (uint8);
}

/// @notice Deployment script for SSOT v1.4 bank observability router/pool topology.
///
/// Required env:
...
///
/// Example:
///   forge script script/DeployV14.s.sol:DeployV14 --rpc-url $RPC_URL --broadcast -vvv
contract DeployV14 is Script {
    ...
    function run() external {
        ...
        if (_shouldWriteArtifacts()) {
            ...
            console2.log("Skipping v1.4 deployment artifact writes during dry run.");
    ...
    function _writeArtifacts(DeployConfig memory cfg, PoolConfig[] memory pools, Deployed memory d) internal {
        ...
        json = vm.serializeString(obj, "architectureVersion", "v1.4-bank-observability");
        ...
        string memory tag = string.concat(vm.toString(block.chainid), "-", vm.toString(block.number), "-v14");
        string memory snapPath = string.concat("deployments/snapshots/deploy-", tag, ".json");

        _safeWriteJson(json, snapPath);
        _safeWriteJson(json, "deployments/latest-v14.json");
        console2.log("Wrote v1.4 deployment snapshot:", snapPath);
        console2.log("Wrote v1.4 deployment snapshot:", "deployments/latest-v14.json");
        ...
        _safeWriteFile("deployments/verify-latest-v14.sh", sh);
        console2.log("Wrote v1.4 verify helper:", verifyPath);
        console2.log("Wrote v1.4 verify helper:", "deployments/verify-latest-v14.sh");
    }
    ...
    function _tryAssetMetadata(address token) internal view returns (string memory sym, uint8 dec) {
        ...
        try IERC20MetadataLikeV14(token).symbol() returns (string memory s) {
            sym = s;
        } catch {}
        try IERC20MetadataLikeV14(token).decimals() returns (uint8 d) {
            dec = d;
        } catch {}
    }
