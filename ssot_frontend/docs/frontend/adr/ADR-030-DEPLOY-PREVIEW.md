...
Use Vercel CLI in a GitHub Actions workflow to deploy preview URLs on every PR that touches `ssot_frontend/`.
...
2. **PR path filter** — only triggers on changes in `ssot_frontend/` to avoid unnecessary deploys for Solidity-only PRs.
