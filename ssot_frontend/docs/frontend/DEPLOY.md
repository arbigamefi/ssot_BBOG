...
2. Set **Root Directory** to `ssot_frontend`
...
Since `ssot_frontend/` is a subdirectory of a larger repo, configure Vercel:

- **Root Directory**: `ssot_frontend`
- **Ignored Build Step**: Use the default (Vercel auto-detects changes in the root directory)

### Build Settings

The `vercel.json` file (if needed) should be placed at `ssot_frontend/vercel.json`:
...
Place at `ssot_frontend/Dockerfile`:
...
```bash
docker build -t ssot-frontend -f ssot_frontend/Dockerfile ssot_frontend/
