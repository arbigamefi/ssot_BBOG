...
describe("AppShell", () => {
  afterEach(() => cleanup());
  it("renders the SSOT v2 brand link", () => {
    render(<AppShell><div>content</div></AppShell>);
    expect(screen.getByText("SSOT v2")).toBeDefined();
  });

  it("renders all 7 nav links", () => {
    render(<AppShell><div>content</div></AppShell>);
    const expectedLinks = ["Games", "Bets", "Liquidity", "Claims", "Referral", "Account", "Ops"];
  ...
  it("renders at least one ReleaseBadge", () => {
    render(<AppShell><div>content</div></AppShell>);
    const badges = screen.getAllByTestId("release-badge");
    expect(badges.length).toBeGreaterThanOrEqual(1);
    expect(badges[0]!.textContent).toBe("test-net");
  });
