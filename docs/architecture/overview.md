...
- **Minimized trust surface:** Bank trusts only Hub; VRFHub is transport-only; games are pure modules.
...
- Hub-only bet funds API (`holdBet/settleBet/refundBet`)
...
4. Hub calls `Bank(asset).holdBet(betId, player, stake, reserved, snapshotHash)`.
...
5. Hub resolves payout via module and calls `Bank(asset).settleBet(...)`.
