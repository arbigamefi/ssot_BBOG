...
| LP          | 单资产份额与受限赎回           | 受每池暂停、准备金和出金缓冲约束；网站主网存入关闭，取出不受该开关影响 | 不保证正收益、随时退出或固定 APY；v1.5 中 LP 不获得庄家优势份额 |
...
2026-09-26 起，主网 LP 存入另有独立开关 `NEXT_PUBLIC_LP_DEPOSITS_ENABLED=false`，打开下注不会连带打开存入。原因是 v1.5 的 GameHub 按已使用流水把全部庄家优势计为协议费和推荐负债，LP 只承担结果波动：Base 主网区块 51799339 的 USDC Bank 协议费计提正好是已使用流水的 2.000%，推荐负债为 0。该开关同样不是链上暂停，合约仍可被直接调用；链上暂停会同时冻结取出，因此没有采用。
