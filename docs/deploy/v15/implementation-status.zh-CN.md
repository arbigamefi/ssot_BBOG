...
当前状态入口已迁至[发布事实表](../../release/STATUS-v1.5.zh-CN.md)（2026-09-25 核验）。主网八款各一笔真实单轮验收已完成，网站主网投注仍关闭；两条链的新 USDC Bank 本次回读均未暂停，WETH Bank 仍暂停。

本文以下按实施顺序保留历史记录。旧段落中的“尚未部署／未下注／预算待明确／最终暂停／待恢复”等属于该阶段快照，不能当作当前状态；不要重放其中已执行的操作包。具体当前值、证据范围和后续待办以事实表与[系统复盘](../../audit/RepositoryReview-2026-09-25.zh-CN.md)为准。
...
## 公网 TLS 与诊断部署完成（2026-09-24 07:59 UTC）

PR #66 全部检查通过并合并，三个域名的正式证书均已签发，VPS loopback 严格验证通过，Cloudflare 已仅对三个应用主机名启用严格回源。公网主站 200、别名 301 保留路径参数、双链健康状态正常；监控诊断已由原 timer 实际生成。五个容器均未重启，旧证书文件和失效配置项已清理。详见[部署验收记录](public-endpoint-closeout.zh-CN.md)。上节“候选待部署”被本次证据取代；间歇性超时、用户主网业务执行和历史 Bank 暂停仍按各自边界保留待办。

## 八款游戏主网单轮验收完成（2026-09-24）

用户已在自己的终端执行新增 4 USDC LP、精确 0.057264 USDC 投注授权及八款游戏的单轮投注。LP 份额合计 6,000,000。每笔均核对双 RPC 的正式区块哈希、发送者、目标、calldata、nonce、真实 VRF 回调、生产 keeper 自动结算、USDC 转账和 PostgreSQL 凭证；没有人工 finalize。详细交易及状态见[真实主网验收证据](base-mainnet-live-acceptance.json)。上文“未新增 LP 或投注”为历史状态。

| 仓位 | 游戏      | 投注 USDC | 净到账 USDC | Keeper 处理耗时 |
| ---- | --------- | --------- | ----------- | --------------- |
| 1    | dice      | 0.003000  | 0.000000    | 3141 ms         |
| 2    | coin-toss | 0.010000  | 0.019600    | 4103 ms         |
| 3    | roulette  | 0.002702  | 0.000000    | 3111 ms         |
| 4    | keno      | 0.010000  | 0.007350    | 4087 ms         |
| 5    | plinko    | 0.010000  | 0.006755    | 3108 ms         |
| 6    | sic-bo    | 0.010000  | 0.000000    | 4124 ms         |
| 7    | slots     | 0.001562  | 0.000000    | 3091 ms         |
| 8    | baccarat  | 0.010000  | 0.000000    | 3125 ms         |

最终双 RPC 汇总核验区块为 **51733984**：累计投注 **0.057264 USDC**、净到账 **0.033705 USDC**、玩家净亏损 **0.023559 USDC**。钱包余额 **1.076441 USDC**；Bank 现金 **6.023559 USDC**，其中协议费 **0.001145 USDC**，净资产 **6.022414 USDC**。LP 总份额及部署钱包份额均为 **6,000,000**；reserved=0、剩余投注授权=0、nextPositionIdHint=9、部署钱包 nonce=65。自资金准备基线以来钱包 ETH 减少 **0.000164597845297614 ETH**，低于 **0.0004 ETH** 上限；USDC 损益不含 ETH 费用。

这证明八款计划中的单轮真实 VRF/自动结算路径，包括 Coin Toss 的中奖派彩；不等于所有玩法、退款/多轮退出或真实手机钱包跳转均已验收。公开前端 `NEXT_PUBLIC_CASINO_RISK_IN_ENABLED=false` 仍保持不变，该开关不构成合约暂停。

移动钱包入口和主网可用性提示由 PR #69 实现，八项 PR 检查通过后合并为 `cb74e098a3d40368871e56e0bcafd4373861dd6c`，合并后的 CI、前端 CI 及正式镜像构建全部通过。本地严格测试 871 项通过、13 项 PostgreSQL 集成测试因未配置数据库跳过；CI 配置真实 PostgreSQL 执行完整检查。浏览器检查覆盖 390px/1440px 布局、连接前主网提示、禁用下注、切到 Base Sepolia 后恢复连接入口。真实 iOS/Android 钱包应用跳转仍需设备验收。

生产切换已完成：Web/keeper 不可变镜像来源提交和双链 v1.5 manifest 一致，配置检查通过。确认双链 keeper 健康且队列为空后，仅重建三个应用容器；PostgreSQL 与 Caddy 的容器 ID 和启动时间未变。切换后双链与公网健康均为 ok、队列为 0，八笔历史凭证仍从 PostgreSQL 读取且金额、终态和结算哈希逐笔一致。浏览器已看到新的主网提示和禁用按钮，未观察到 console error/warn。见[生产切换证据](pr69-production-release.json)。公开下注仍关闭，手机钱包跳转验收待用户设备反馈。
