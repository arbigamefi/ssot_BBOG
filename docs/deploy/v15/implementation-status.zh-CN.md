...
## 主网真实业务验收启动（2026-09-24 13:13 UTC）

用户已亲自在本机执行新增 4 USDC LP、精确 0.057264 USDC 投注授权和第一笔 0.003 USDC Dice。LP 份额合计 6,000,000。两家 RPC 核对正式区块哈希、发送者、目标、calldata、nonce 和状态均一致；不以即时回执中的零 blockHash 作为确认依据。详见[真实主网验收证据](base-mainnet-live-acceptance.json)。上文“未新增 LP 或投注”为历史状态。

Dice 仓位 1 在区块 51733077 建立，51733081 收到真实 VRF 回调，51733082 由生产 keeper `0x440558699040d28975D218caB497338B65960A92` 自动结算。keeper 处理耗时 3141 ms；没有人工 finalize。本轮净派彩 0、协议费计提 0.000060 USDC、Bank 净资产 6.002940 USDC、reserved=0、玩家余额 1.097 USDC。PostgreSQL 和[公开凭证](https://arbigamefi.com/casino/receipt/8453/1)与链上一致。

这只验收了 Dice 的一笔亏损及自动结算，其他七款游戏及中奖派彩路径仍待完成。公开前端 `NEXT_PUBLIC_CASINO_RISK_IN_ENABLED=false` 保持不变；该前端开关不构成合约暂停。移动钱包入口与主网可用性提示的前端候选已完成 107 个测试文件、542 项测试、lint、类型检查、生产构建及包体门禁；浏览器检查覆盖 390px/1440px 布局、主网禁止投注、切到 Base Sepolia 后恢复连接入口。真实手机钱包应用跳转尚未验收，候选尚未部署。
