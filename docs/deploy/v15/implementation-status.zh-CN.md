...
更新于 2026-09-23。Sepolia v1.5 的真实投注、部分退款、恢复、guardian 与 2/3 Safe 演练已完成本次明确范围，两个测试 Bank 最终均暂停，测试 LP 与协议费保留用于回归。Base 主网 v1.5 已完成 51 笔部署交易，并经双 RPC 逐笔核对；当前待真实 Safe 接收治理及签名发布。公开 Web 与主网 keeper 仍使用旧合约，新应用切换和旧系统退役尚未完成。下文早期阶段记录中的余额和待办均为当时快照。
...
| 5. Base 主网部署            | 已部署，待 Safe 治理接收                  | 复用通过验收的源代码和主网参数；核验成本与 nonce；部署、Safe 接收、链上状态及签名发布包验证完成                                |
...
下一步由两名真实 Safe owner 审阅、签署并执行主网接收包。随后核验 Safe 成功事件、7 项治理接收事件及 `pendingGovernance=0`，再签署发布元数据、导入主网 manifest、构建双链不可变镜像。当前治理尚未接收、两个 Bank 保持暂停，公开应用未切换。
