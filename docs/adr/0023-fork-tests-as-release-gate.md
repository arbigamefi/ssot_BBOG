...
## Implementation
- Release CI runs `script/ci/fork_release_gate.sh`, which reads the deployed `chainId` and `vrfWrapper` from `deployments/latest.json` and executes `test/fork/*` with `FORK_REQUIRED=1`.
